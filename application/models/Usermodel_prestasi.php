...
class Usermodel_prestasi extends CI_Model {
	...
	$this->polinema_wsec->from('prestasi');
	...
	function add($data)
	{
	$this->polinema_wsec->insert('prestasi',$data);
	}
	
	function edit($id,$data)
	{
	$this->polinema_wsec->where('id',$id);
	$this->polinema_wsec->update('prestasi',$data);
	}
	function delete($id)
	{
	$this->polinema_wsec->where('id',$id);
	$this->polinema_wsec->delete('prestasi');
	}	
}